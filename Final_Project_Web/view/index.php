...
if (isset($_POST["addcart"])) {
  for ($i = 0; $i < count($result); $i++) {
    if ($_POST["addcart"] == $result[$i]['id']) {
      ...
      $sql = "INSERT into cart (id,name,price,color,type,quantity,total) values(null,'" . $name . "'," . $price . ",'" . $color . "','" . $type . "'," . $quantity . "," . $total . ")";
      var_dump(($sql));
      $db->query($sql);
      header("refresh:0");
    }
          ...
          <div class="grid-item">
            ...
            <p>
              ...
              <form action="" method="post">
                <button style=" color:red; border: none" ; class="fas fa-shopping-cart" name="addcart" value="<?php echo  $clothes[$i]->id; ?>"></button>
              </form>
          </div>
        <?php } ?>
      </div>
